 ...
 * Description: Rebuilds Home, About, Services, Contact as a premium consulting agency. DELETE after running.
 * Version: 11.0
 */
...
add_filter('plugin_row_meta', function($links, $file) {
    if (strpos($file, 'brndguru-fixes') !== false) {
        ...
        $links[] = '<a href="' . admin_url('?bg11_run=1&bg11_nonce=' . $nonce) . '" style="font-weight:700;color:#e4b84d;font-size:14px;">▶ REBUILD SITE AS CONSULTING AGENCY</a>';
    }
    return $links;
}, 10, 2);
...
add_action('admin_init', function() {
    ...
    echo str_repeat('=', 60) . "\n";
    echo '<span style="color:#e4b84d;font-size:15px;">✓ DONE. Your site now looks like a consulting agency.</span>' . "\n";
...
function bg11_head($m) { echo '<span style="color:#e4b84d">── ' . esc_html($m) . ' ──</span>' . "\n"; }
...
function bg11_btn(string $id, string $text, string $url, string $bg='#e4b84d', string $color='#000000', string $align='left', string $size='lg'): array {
...
function bg11_card(string $icon, string $title, string $desc, string $url=''): array {
    static $i=0; $i++;
    $children = [
        bg11_text('ci-'.$i, '<div style="font-size:36px;margin-bottom:16px;">'.$icon.'</div>'),
        bg11_heading('ct-'.$i, $title, 'h3', '#ffffff', 22, 'left', '700'),
        bg11_text('cd-'.$i, '<p style="color:#999;font-size:15px;line-height:1.7;margin:12px 0 0;">'.$desc.'</p>'),
    ];
    if ($url) $children[] = bg11_btn('cb-'.$i, 'Learn More →', $url, 'transparent', '#e4b84d', 'left', 'sm');
    return bg11_container('card-'.$i, [
        ...
        'box_shadow_box_shadow'=>['horizontal'=>0,'vertical'=>4,'blur'=>24,'spread'=>0,'color'=>'rgba(0,0,0,0.4)'],
    ], $children);
}
function bg11_stat(string $num, string $label): array {
    ...
    return bg11_container('stat-'.$i,[
        ...
        bg11_heading('sn-'.$i,$num,'h3','#e4b84d',52,'center','800'),
...
function bg11_step(int $num, string $title, string $desc): array {
    ...
    return bg11_container('step-'.$i,[
        ...
        bg11_text('stepn-'.$i,'<div style="font-size:48px;font-weight:800;color:#e4b84d;opacity:.4;line-height:1;">'.(str_pad($num,2,'0',STR_PAD_LEFT)).'</div>'),
...
function bg11_home() {
    ...
    $el[] = bg11_container('hero', [
        ...
        'padding'=>['unit'=>'px','top'=>'140','right'=>'40','bottom'=>'120','left'=>'40','isLinked'=>false],
        'text_align'=>'left',
    ],[
        bg11_inner([
            bg11_container('hero-left',[
                'flex_direction'=>'column','width'=>['unit'=>'%','size'=>60],'flex'=>'1 1 400px',
            ],[
                bg11_text('hero-tag','<span style="color:#e4b84d;font-size:13px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">Brand & Digital Consulting</span>'),
                bg11_spacer('hs1',16),
                bg11_heading('hero-h1','We Build Brands That Dominate Markets.','h1','#ffffff',62,'left','800'),
                bg11_spacer('hs2',24),
                bg11_text('hero-sub','<p style="color:#aaaaaa;font-size:20px;line-height:1.7;max-width:580px;">BrndGuru is a results-driven consulting agency that transforms ambitious companies into category-defining brands. Strategy, design, and digital — all under one roof.</p>'),
                bg11_spacer('hs3',40),
                bg11_container('hero-btns',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>16],'flex_wrap'=>'wrap'],[
                    bg11_btn('hero-btn1','Book a Strategy Call →',$BOOK,'#e4b84d','#000000','left','lg'),
                    bg11_btn('hero-btn2','See Our Work',get_home_url().'/portfolio/','transparent','#ffffff','left','lg'),
                ]),
                bg11_spacer('hs4',60),
                bg11_text('hero-trust','<div style="display:flex;gap:40px;flex-wrap:wrap;align-items:center;">
                    <span style="color:#666;font-size:13px;letter-spacing:1px;text-transform:uppercase;">Trusted by brands in</span>
                    <span style="color:#aaa;font-size:15px;font-weight:600;">E-Commerce</span>
                    <span style="color:#aaa;font-size:15px;font-weight:600;">SaaS</span>
                    <span style="color:#aaa;font-size:15px;font-weight:600;">Retail</span>
                    <span style="color:#aaa;font-size:15px;font-weight:600;">Healthcare</span>
                    <span style="color:#aaa;font-size:15px;font-weight:600;">Fintech</span>
                </div>'),
            ]),
        ], 1200, 'row', '60'),
    ]);
    ...
    $el[] = bg11_container('stats-bar',[
        ...
        'padding'=>['unit'=>'px','top'=>'48','right'=>'40','bottom'=>'48','left'=>'40','isLinked'=>false],
        'border_top_width'=>['unit'=>'px','top'=>'1','right'=>'0','bottom'=>'0','left'=>'0','isLinked'=>false],
        'border_color'=>'#222222','flex_wrap'=>'wrap','gap'=>['unit'=>'px','size'=>0],
    ],[
        bg11_inner([
            bg11_stat('150+','Projects Delivered'),
            bg11_stat('8+','Years of Expertise'),
            bg11_stat('40+','Brand Identities Built'),
            bg11_stat('98%','Client Retention Rate'),
        ], 1200, 'row', '0'),
    ]);
    ...
    $el[] = bg11_section('#0a0a0a',[
        bg11_inner([
            bg11_text('svc-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">What We Do</span>'),
        ], 1200, 'column', '0'),
        ...
        bg11_inner([
            bg11_btn('svc-cta','View All Services →',$home.'/services/','#e4b84d','#000000','center','lg'),
        ], 1200, 'column', '0'),
    ], 100, 100);
    ...
    $el[] = bg11_section('#141414',[
        bg11_inner([
            bg11_container('why-left',['flex_direction'=>'column','width'=>['unit'=>'%','size'=>48],'flex'=>'1 1 360px'],[
                bg11_text('why-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">Why BrndGuru</span>'),
                ...
                bg11_btn('why-btn','Schedule a Consultation',$BOOK,'#e4b84d','#000000','left','lg'),
            ]),
    ...
    $el[] = bg11_section('#0d0d0d',[
        bg11_inner([
            bg11_text('proc-tag','<div style="text-align:center;"><span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">How We Work</span></div>'),
        ],1200,'column','0'),
    ...
    $el[] = bg11_container('cta-banner',[
        'flex_direction'=>'column','content_width'=>'full',
        'background_background'=>'classic','background_color'=>'#e4b84d',
        'padding'=>['unit'=>'px','top'=>'90','right'=>'40','bottom'=>'90','left'=>'40','isLinked'=>false],
        'text_align'=>'center',
    ],[
        bg11_heading('cta-h','Ready to Build a Brand That Leads?','h2','#000000',46,'center','800'),
        bg11_spacer('cta-sp1',20),
        bg11_text('cta-sub','<p style="color:#333;font-size:19px;line-height:1.6;max-width:580px;margin:0 auto;">Book a free 30-minute strategy call. We\'ll identify your biggest growth opportunity and show you exactly how we\'d approach it.</p>','center'),
        bg11_spacer('cta-sp2',36),
        bg11_container('cta-btns',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'16'],'justify_content'=>'center','flex_wrap'=>'wrap'],[
            ...
            bg11_btn('cta-btn2','View Case Studies',get_home_url().'/portfolio/','transparent','#000000','center','lg'),
        ]),
    ]);
...
function bg11_about() {
    ...
    $el[] = bg11_container('ab-hero',[
        ...
        bg11_inner([
            bg11_container('ab-hero-content',['flex_direction'=>'column','width'=>['unit'=>'%','size'=>65],'flex'=>'1 1 400px'],[
                bg11_text('ab-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">About BrndGuru</span>'),
                ...
                bg11_btn('ab-cta','Work With Us',$BOOK,'#e4b84d','#000000','left','lg'),
            ]),
        ],1200,'row','0'),
    ]);
    ...
    $el[] = bg11_section('#111111',[
        bg11_inner([
            bg11_container('ms-left',['flex_direction'=>'column','flex'=>'1 1 340px','width'=>['unit'=>'%','size'=>40]],[
                bg11_text('ms-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">Our Mission</span>'),
    ...
    $el[] = bg11_section('#0a0a0a',[
        bg11_inner([
            bg11_text('val-tag','<div style="text-align:center;"><span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">Our Values</span></div>'),
        ],1200,'column','0'),
    ...
    $el[] = bg11_section('#141414',[
        bg11_inner([
            bg11_container('hw-left',['flex_direction'=>'column','flex'=>'1 1 340px','width'=>['unit'=>'%','size'=>45]],[
                bg11_text('hw-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">How We Work</span>'),
                ...
                bg11_btn('hw-btn','Start a Conversation',$BOOK,'#e4b84d','#000000','left','lg'),
            ]),
            bg11_container('hw-right',['flex_direction'=>'column','flex'=>'1 1 340px','width'=>['unit'=>'%','size'=>50],'gap'=>['unit'=>'px','size'=>'4']],[
                bg11_container('hw-item1',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'20'],'padding'=>['unit'=>'px','top'=>'20','right'=>'0','bottom'=>'20','left'=>'0','isLinked'=>false],'border_bottom_width'=>['unit'=>'px','top'=>'0','right'=>'0','bottom'=>'1','left'=>'0','isLinked'=>false],'border_color'=>'#222'],[
                    bg11_text('hwi1n','<span style="font-size:22px;font-weight:800;color:#e4b84d;min-width:32px;">01</span>'),
                ...
                bg11_container('hw-item2',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'20'],'padding'=>['unit'=>'px','top'=>'20','right'=>'0','bottom'=>'20','left'=>'0','isLinked'=>false],'border_bottom_width'=>['unit'=>'px','top'=>'0','right'=>'0','bottom'=>'1','left'=>'0','isLinked'=>false],'border_color'=>'#222'],[
                    bg11_text('hwi2n','<span style="font-size:22px;font-weight:800;color:#e4b84d;min-width:32px;">02</span>'),
                ...
                bg11_container('hw-item3',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'20'],'padding'=>['unit'=>'px','top'=>'20','right'=>'0','bottom'=>'20','left'=>'0','isLinked'=>false],'border_bottom_width'=>['unit'=>'px','top'=>'0','right'=>'0','bottom'=>'1','left'=>'0','isLinked'=>false],'border_color'=>'#222'],[
                    bg11_text('hwi3n','<span style="font-size:22px;font-weight:800;color:#e4b84d;min-width:32px;">03</span>'),
                ...
                bg11_container('hw-item4',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'20'],'padding'=>['unit'=>'px','top'=>'20','right'=>'0','bottom'=>'20','left'=>'0','isLinked'=>false]],[
                    bg11_text('hwi4n','<span style="font-size:22px;font-weight:800;color:#e4b84d;min-width:32px;">04</span>'),
...
function bg11_services() {
    ...
    $el[] = bg11_container('sv-hero',[
        ...
        bg11_text('sv-tag','<div style="text-align:center;"><span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">What We Offer</span></div>'),
        ...
        bg11_btn('sv-cta','Book a Strategy Call →',$BOOK,'#e4b84d','#000000','center','lg'),
    ]);
        ...
        $el[] = bg11_section($bg,[
            bg11_inner([
                ...
                bg11_container('svr-content-'.$i,['flex_direction'=>'column','flex'=>'1 1 400px'],[
                    ...
                    bg11_btn('svr-btn-'.$i,'Explore This Service →',$s[3],'#e4b84d','#000000','left','md'),
                ]),
            ],1200,'row','48'),
        ],72,72);
    }
    ...
    $el[] = bg11_container('ct-hero',[
        ...
        bg11_inner([
            bg11_container('ct-left',['flex_direction'=>'column','flex'=>'1 1 380px','width'=>['unit'=>'%','size'=>48]],[
                bg11_text('ct-tag','<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">Get In Touch</span>'),
                ...
                bg11_container('ct-info',['flex_direction'=>'column','gap'=>['unit'=>'px','size'=>'24']],[
                    ...
                    bg11_container('ct-i2',['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'16'],'align_items'=>'center'],[
                        ...
                        bg11_container('ct-i2t',['flex_direction'=>'column'],[
                            ...
                            bg11_text('ct-i2v','<a href="'.$BOOK.'" target="_blank" style="color:#e4b84d;font-size:16px;font-weight:600;text-decoration:none;">Schedule via Calendly →</a>'),
                        ]),
                    ]),
            ...
            bg11_container('ct-right',['flex_direction'=>'column','flex'=>'1 1 380px','width'=>['unit'=>'%','size'=>48],
                ...
                bg11_btn('ct-r-btn1','Schedule Your Call Now →',$BOOK,'#e4b84d','#000000','center','lg'),
    ...
    foreach ($pages as $pid => $p) {
        ...
        $el[] = bg11_container('sp-hero-'.$pid,[
            ...
            bg11_inner([
                bg11_container('sp-hero-c-'.$pid,['flex_direction'=>'column','flex'=>'1 1 400px','width'=>['unit'=>'%','size'=>65]],[
                    ...
                    bg11_text('sp-tag-'.$pid,'<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">BrndGuru Consulting</span>'),
                    bg11_spacer('sp-sp1-'.$pid,12),
                    bg11_heading('sp-h-'.$pid,$title,'h1','#ffffff',52,'left','800'),
                    bg11_spacer('sp-sp2-'.$pid,12),
                    bg11_heading('sp-tl-'.$pid,$tagline,'h2','#e4b84d',24,'left','500'),
                    ...
                    bg11_container('sp-btns-'.$pid,['flex_direction'=>'row','gap'=>['unit'=>'px','size'=>'16'],'flex_wrap'=>'wrap'],[
                        bg11_btn('sp-btn1-'.$pid,'Start This Project',$BOOK,'#e4b84d','#000000','left','lg'),
        ...
        foreach ($deliverables as $d) {
            $items .= '<div style="display:flex;align-items:center;gap:12px;padding:14px 0;border-bottom:1px solid #1e1e1e;">
                <span style="color:#e4b84d;font-size:18px;flex-shrink:0;">✓</span>
        ...
        $el[] = bg11_section('#111111',[
            bg11_inner([
                bg11_container('sp-del-left-'.$pid,['flex_direction'=>'column','flex'=>'1 1 340px','width'=>['unit'=>'%','size'=>45]],[
                    bg11_text('sp-del-tag-'.$pid,'<span style="color:#e4b84d;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;">What\'s Included</span>'),
                    ...
                    bg11_btn('sp-del-btn-'.$pid,'Discuss Your Project →',$BOOK,'#e4b84d','#000000','left','lg'),
                ]),
